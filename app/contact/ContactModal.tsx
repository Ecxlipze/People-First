      ...
      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6">
          ...
          <button
            ...
            tabIndex={-1}
            onClick={close}
            className="animate-fade-in-up absolute inset-0 cursor-default bg-black/45 backdrop-blur-[3px]"
          />
          ...
          <div
            ...
            tabIndex={-1}
            className="animate-fade-in-up relative max-h-[92dvh] w-full max-w-5xl overflow-y-auto overscroll-contain rounded-xl shadow-[0_40px_100px_-20px_rgba(0,0,0,0.7)] outline-none"
          >
            <button
              type="button"
              onClick={close}
              aria-label="Close"
              className="absolute right-3 top-3 z-10 rounded-full bg-white/15 p-2 text-white/90 transition-colors hover:bg-white/25 hover:text-white"
            >
              <X className="h-5 w-5" />
            </button>
